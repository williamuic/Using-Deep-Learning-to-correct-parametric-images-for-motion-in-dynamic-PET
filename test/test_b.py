...
if __name__ == '__main__':
    ...
    dataset = create_dataloader(opt.dataroot, opt.data_type, 'Vd', opt.batch_size, False, transform=None, global_norm=opt.global_norm, no_norm=opt.no_norm)  # create a dataset given opt.dataset_mode and other options
    model = create_model(opt)      # create a model given opt.model and other options
    model.setup(opt)               # regular setup: load and print networks; create schedulers
    print(dataset.num_workers)

    # Create directory if it doesn't exist
    if not os.path.exists('test_results_b'):
        os.makedirs('test_results_b')
    ...
    for i, data in enumerate(dataset):
        ...
        np.save(f'test_results_b/real_A_{i}.npy', real_A_denorm.cpu().numpy())
        np.save(f'test_results_b/fake_B_{i}.npy', output_denorm.detach().cpu().numpy())
        np.save(f'test_results_b/real_B_{i}.npy', real_B_denorm.cpu().numpy())
